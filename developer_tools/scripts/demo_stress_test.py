...
# --- CONFIGURAÇÃO ---
BASE_URL = os.getenv("API_URL", "http://localhost:9999") # Defaulting to localhost for local test
LOGIN_URL = f"{BASE_URL}/login"
BATCH_URL = f"{BASE_URL}/api/churn/batch/optimized"
INPUT_FILE = r"docs\csv\clientes_teste_stress_5min.csv"
...
def print_header():
    clear_screen()
    print(f"{C_BLUE}" + "="*80)
    print(f"{C_BOLD}   CHURN INSIGHT AI - STRESS TEST (5 MIN){C_RESET}{C_BLUE}")
    print(f"   Arquitetura: Híbrida (OCI + On-Premise)")
    print(f"   Target: ~3500 registros")
    print("="*80 + f"{C_RESET}")
    print()
...
def process_batch(token, filename):
    ...
    print(f"   Estratégia       : {C_YELLOW}Processamento Otimizado{C_RESET}")
    print("-" * 80)
    ...
    with open(filename, 'rb') as f:
        ...
        try:
            ...
            # Increase timeout for large files if needed, though optimized endpoint should be fast-ish or async
            response = requests.post(BATCH_URL, files=files, headers=headers, timeout=300) 
            ...
            if response.status_code == 200:
                ...
                # Faster visualization for stress test (0.01 instead of 0.05)
                # Or maybe print every N lines to avoid console spam?
                # Let's keep it visible but fast.
                ...
                for line in lines:
                    ...
                    try:
                        idx_valor = header_map.get("valorMensal", 5) 
                    ...
                    # Visualization optimization: Print only every 10th or just faster?
                    # Printing 3500 lines will take time on console IO.
                    # Let's reduce delay and verify console speed.
                    
                    # if count % 10 == 0:
                    #     sys.stdout.write(f"\r   Processado: {count}/{len(lines)-1}")
                    #     sys.stdout.flush()
                    
                    # Actually user wanted to "see" it processed, let's print but very fast.
                    color = C_RED if is_churn else C_GREEN
                    icon = "🚨" if is_churn else "✅"
                    status = "ALERTA" if is_churn else "SEGURO"
                    
                    # Simplified line for speed
                    # print(f"   {color}{parts[0]:<10} | {prob:.4f} | {icon} {status}{C_RESET}")
                    # time.sleep(0.001) 
                
                # To really fit 5 mins while showing "processing", we can rely on the backend response time mostly,
                # but if we want to simulate the 'streaming' effect locally from the response text:
                
                print(f"   Processamento finalizado. {len(lines)-1} registros analisados.")
...
def print_report(churn, safe, revenue, prob_sum, total):
    print("\n")
    print(f"{C_BLUE}" + "="*80)
    print(f"{C_BOLD}   RELATÓRIO DE ESTRESSE{C_RESET}")
    ...
    print(f"   📉 Taxa de Risco : {C_YELLOW}{churn_rate:.1f}%{C_RESET}")
    print(f"   📊 Score Médio   : {C_CYAN}{avg_prob:.1f}%{C_RESET}")
    print("-" * 40)
...
def main():
    ...
    churn, safe, revenue, prob_sum, total = process_batch(token, INPUT_FILE)
