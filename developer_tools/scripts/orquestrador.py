...
def print_banner():
    clear_screen()
    print(f"{C_BLUE}" + "="*80)
    print(f"{C_BOLD}   CHURN INSIGHT - ORQUESTRADOR DE DEMONSTRAÇÃO{C_RESET}")
    print(f"   {C_CYAN}Ambiente: OCI (Oracle Cloud Infrastructure){C_RESET}")
...
def generate_chunk(start_id, count):
    ...
    csv_lines = []
    # Header apenas se for o primeiro bloco? API precisa de header sempre.
...
def run_stress_test():
    ...
    TARGET_TOTAL = 50000  # Meta para 8 minutos
    CHUNK_SIZE = 2000     # Tamanho do lote por requisição (equilíbrio entre timeout e overhead)
    ...
    try:
        while total_processed < TARGET_TOTAL:
            ...
            # Criar arq temporário (requests precisa)
            ...
            # 3. Processar Resposta (Streaming Visual)
            if resp.status_code == 200:
                ...
                # Efeito Matrix: Mostra alguns logs aleatórios do lote para dar sensação de velocidade
                sample_size = min(10, len(data_lines)) # Mostra 10 por lote para não travar o terminal
                
                print(f"\r✅ Lote processado em {batch_time:.2f}s | Speed: {current_chunk_size/batch_time:.0f} rec/s")
                
                for line in data_lines:
                    # Lógica de contagem
                    # Formato output esperado: id, ..., previsao, prob, risco
                    if "ALTO" in str(line).upper() or "0.428" in line or "0.5" in line: # Simplificação rápida
                         churn_in_batch += 1
...
def main():
    while True:
        ...
        if opt == "1":
            # Roda o bat existente no contexto da pasta dele
            subprocess.call([r"run_tests.bat", "OCI"], cwd=r"developer_tools\scripts", shell=True)
